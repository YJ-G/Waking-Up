...
* 进程和线程
    ...
    * [进程同步问题](#进程同步问题)
    * [进程有哪几种状态？](#进程有哪几种状态)
    * [上下文切换（context switching）](#上下文切换（context switching）)
    * [进程调度策略有哪些？](#进程调度策略有哪些)
    * [什么是僵尸进程？](#什么是僵尸进程)
    * [线程同步有哪些方式？](#线程同步有哪些方式)
    * [什么是协程？](#什么是协程)
...
进程是资源分配的基本单位，但是线程只拥有一点在运行中必不可少的资源(如程序计数器,一组寄存器和栈)，线程可以访问隶属进程的资源。
...
线程间可以通过直接读写同一进程中的数据进行通信，但是进程通信需要借助 进程间通信（IPC）。
...
上下文切换指的是内核（操作系统的核心）在CPU上对进程或者线程进行切换。上下文切换过程中的信息被保存在进程控制块（PCB-Process Control Block）中。

### 进程调度策略有哪些？

从就绪队列中调出一个进程/线程作为cpu将要运行的下一个进程/线程，就是进程/线程调度
